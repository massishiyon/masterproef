...
#%% Before eliminating discrimination in test set, calculate chance of having income >50K given each sex and
# discrimination
print("Before eliminating discrimination in test set:")
# Chance of having income >50K given being male for test set
highincome_male_prob_test = df.loc[indices_test].loc[(df.sex == True) & (df.income == True)].shape[0] / \
...
highincome_female_prob_test = df.loc[indices_test].loc[(df.sex == False) & (df.income == True)].shape[0] / \
...
#%% Remove discrimination in test set through preferential sampling to simulate non-discriminatory test set
df, indices_test, nd_x_test, nd_y_test = tch.preferential_sampling(df, indices_test, d_x_test, d_y_test, "GNB")[0:4]

#%% After eliminating discrimination in test set, calculate chance of having income >50K given each sex and
# discrimination
...
#%% Before eliminating discrimination in training set, calculate chance of having income >50K given each sex and
# discrimination
print("Before eliminating discrimination in training set:")
# Chance of having income >50K given being male for training set
highincome_male_prob_train = df.loc[indices_train].loc[(df.sex == True) & (df.income == True)].shape[0] / \
...
highincome_female_prob_train = df.loc[indices_train].loc[(df.sex == False) & (df.income == True)].shape[0] / \
...
df, indices_train, nd_x_train, nd_y_train, gnb_d_clf = tch.massaging(df, indices_train, d_x_train, d_y_train, "GNB")

#%% After eliminating discrimination in training set, calculate chance of having income >50K given each sex and
# discrimination
...
# Train non-discriminatory model (on massaged dataset)
param_grid = {
